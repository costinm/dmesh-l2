...
int i2c_master_init(nvs_handle_t my_handle)
{
    // TODO: use nvm to get the pins

    int32_t sda = 0; // value will default to 0, if not set yet in NVS
    int err = nvs_get_i32(my_handle, "sda", &sda);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) return err;

    int32_t scl = 0; // value will default to 0, if not set yet in NVS
    err = nvs_get_i32(my_handle, "scl", &scl);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) return err;
    ...
    ESP_LOGI(TAG, "I2C init: %d %d", sda, scl);
