...
void bleAdvtTask(void *pvParameters)
{
    ...
    while (hciRun) {
        ...
        ESP_LOGI(tag, "BLE Advertise, flag_send_avail: %d, cmd_sent: %d\n", send_avail, cmd_cnt);
    }
    ESP_LOGI(tag, "BLE advt task stop\n");
    vTaskDelete(NULL);
}

static bool hciInit = false;

static void start_hci() {
    if (!hciInit) {
...
void register_hci_ble(nvs_handle_t nvs_handle)
{
    ...
    int32_t mode = 0;
    int err =nvs_get_i32(nvs_handle, "hci", &mode);
